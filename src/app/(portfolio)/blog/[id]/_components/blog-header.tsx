...
const BlogHeader = ({ blog }: { blog: DeepReadonlyable<BlogPost> }) => {
    ...
    useEffect(() => {
        ...
        if (!blogElement) throw new Error("Blog content element not found for word count calculation.");

        const blogElementCopy = blogElement.cloneNode(true) as HTMLElement;

        const blogHeader = blogElementCopy.querySelector('#blog-header');
        if (!blogHeader) throw new Error("Blog header element not found for word count calculation.");

        const blogFooter = blogElementCopy.querySelector('#blog-footer');
        if (!blogFooter) throw new Error("Blog footer element not found for word count calculation.");

        blogHeader.remove();
        blogFooter.remove();
        
        blogElementCopy.querySelectorAll('.code-block').forEach((block) => block.remove());

        console.log(blogElementCopy);
        const words: number = countWords(blogElementCopy);
